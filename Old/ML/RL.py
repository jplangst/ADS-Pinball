...
class RL_Controller(object):
    def __init__(self, OUTPUT_RESULTS_DIR='.', MODEL_RESTORE_PATH=None, CHECKPOINT_DIR=None, GAMMA=0.99, LAMBDA=0.95, BATCH=512, MAX_BUFFER_SIZE=20000, INPUT_SHAPE=None):
        ...
        #Used for good start button reward
        self.ballLastSeen = None 
        self.goodStartGiven = False
        
        #Used to check if the ball has not been seen for a certain time
        self.ballGoneSince = None 
        #Used to check if the agent has not performed any actions in a certain time
        self.notPerformedActionSince = None
    ...
    def update_ppo(self):  
        ...
        if self.t >= self.BATCH:
            ...
            #If the experience buffer is to large we reset it. Simple fix for now to include trainnig data from more than one episode
            if self.t > self.maxBufferSize:
    ...
    def calculate_reward(self, ballVisible, currLocation, prevLocation, controllerPenalty, terminal):
        ...
        r = -sharedData.constantPenalty
        # If the controller's trigger limiter script kicked in 
        r -= controllerPenalty

        #If the ball is not visible return the r as is
        if not ballVisible:
            if self.ballLastSeen is not None:
                currTime = datetime.datetime.now()
                timeDiff = currTime-self.ballLastSeen
                if self.a == 4 and timeDiff.total_seconds() > 2:
                    #if not self.goodStartGiven:
                    r += sharedData.goodStartButtonReward
                        #self.goodStartGiven = True 
                    self.ballLastSeen = currTime
                    print("Good start button rewarded")
            if self.a == 0:
                r += sharedData.noActionAndBallNotVisible
            return r
        
        self.goodStartGiven = False

        if self.a == 4:
            r -= sharedData.startButtonBallVisible

        self.ballLastSeen = datetime.datetime.now()
        ...
        if self.goodTrigger:
            if(currLocation[0] < self.prevGoodBallXloc):
                ...
                if(distance > sharedData.goodTriggerBallTravelDistanceThreshold):
                    r += sharedData.goodTriggerReward
                    self.goodTrigger = False 
    ...
    def checkForStagnation(self, ballVisible, action):
        #If the selected action was to do nothing we check how long this has occured
        if action == 0:
            if self.notPerformedActionSince is None:
                self.notPerformedActionSince = datetime.datetime.now()
            elif (self.notPerformedActionSince-datetime.datetime.now()).total_seconds() > sharedData.gameplayStagnationThreshold:
                print("Stagnation due to no actions taken detected!")
                self.pinballController.triggerAction(4)
                time.sleep(0.2)
        ...
        #If the ball is not visible we check how long this has occured
        if not ballVisible:
            if self.ballGoneSince is None:
                self.ballGoneSince = datetime.datetime.now()
            elif (self.ballGoneSince-datetime.datetime.now()).total_seconds() > sharedData.gameplayStagnationThreshold:
                print("Stagnation due to not seeing the ball detected!")
                self.pinballController.triggerAction(4)
                time.sleep(0.2)
                self.pinballController.clearActions()
                return True 
    ...
    def step_environment(self, ballLocationHistory, terminal, episodeState, episode):
        with tf.variable_scope('RLModel'):
            if episodeState == 0: #New episode started
                ...
                print("Starting new episode - ", dt_string)
                self.init_training()
            ...
            sharedData.bGameplayStagnated = self.checkForStagnation(ballVisible, self.a)
            ...
            r = self.calculate_reward(ballVisible, currLocation, prevLocation, controllerPenalty, terminal)
